  ...
  <q-dialog v-model="dialogPostsSort" square full-width>
    <q-card class="q-pl-sm" dark>
      <q-card-section class="row items-center q-pb-none">
        <div class="flex items-center text-body2">{{ title }}</div>
        <q-space />
        <q-btn icon="close" flat round dense @click="toggleDialogPostsSort" />
      </q-card-section>
...
export default defineComponent({
  name: 'DialogPostAdd',
  setup(props) {
    const { dialogPostsSort, toggleDialogPostsSort } = useSettings(props);
