<template>
  ...
  <q-dialog v-model="dialogShow" position="bottom" class="t-post-form">
    ...
    >
      <q-card-section>
        <AddLoginForm @dialog-hide="onDialogHide" />
      </q-card-section>
    </q-card>
  </q-dialog>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const onDialogShow = () => (dialogShow.value = false);
    const onDialogHide = () => (dialogShow.value = false);

    return { menu, showAddPosts, dialogShow, onDialogShow, onDialogHide };
  },
});
</script>
